...
На текущем этапе система **строит карту знаний и выполняет аналитику через LangGraph** (режимы Audit, Hypotheses, Anomalies, обзор литературы, рекомендации). Полноценный contradiction engine и композитный confidence — в доработке.

LangGraph — **аналитический слой поверх** Neo4j + Qdrant + Agent API, не замена worker ingestion/extraction.
