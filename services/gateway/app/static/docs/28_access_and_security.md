...
**Матрица role × classification** — **Настройки → Доступ к данным**, API `GET/PUT /api/v1/settings/data-access` (Postgres `runtime_config.data_access_matrix`).

**Enforcement:** `GET /documents`, `GET /graph/*`, search, RAG — filter on read по `X-MKG-Role`. Без заголовка — `viewer`.

- При upload: поле `classification` (Form, по умолчанию `"открытый"`).
- Postgres `documents.classification`, L5 `SecurityRole.required_clearance`.
...
|-------|------|------------|----------|
| GET | `/api/v1/roles` | Список ролей и флагов `can_*` | ❌ |
